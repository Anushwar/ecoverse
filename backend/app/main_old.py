...
# All data now stored in SQLite database via db_manager
...
async def startup_event():
    ...
    try:
        ...
        dataset_processor.data_dir = data_dir
        await dataset_processor.load_datasets()
        print("✅ Datasets loaded successfully")
    except Exception as e:
        print(f"⚠️  Warning: Could not load datasets: {e}")
        print("📁 Make sure to download datasets to the data/ directory")

@app.get("/datasets/summary")
async def get_dataset_summary():
    """Get summary of loaded datasets"""
    return await dataset_processor.get_dataset_summary()

@app.get("/datasets/insights/{user_id}")
async def get_dataset_insights(user_id: str):
    """Get insights based on user data and datasets"""
    user_activities = activities_db.get(user_id, [])

    recommendations = await dataset_processor.get_personalized_recommendations(user_activities)
    insights = await dataset_processor.get_comparative_insights(user_activities)

    return {
        "dataset_recommendations": recommendations,
        "comparative_insights": insights,
        "data_sources": list(dataset_processor.datasets.keys())
    }

@app.post("/datasets/export")
async def export_dataset_analysis():
    """Export processed dataset analysis"""
    await dataset_processor.export_processed_data()
    return {"message": "Dataset analysis exported to processed_data.json"}
