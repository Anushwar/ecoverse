...
class CarbonAnalysisAgent(BaseAgent):
    ...
    async def execute(self, input_data: dict[str, Any]) -> list[CarbonInsight]:
        ...
        if weekly_trend["trend"] != "stable":
            insights.append(CarbonInsight(
                id=f"trend-{int(datetime.now().timestamp())}",
                user_id=user.id,
                ...
                data=weekly_trend,
                severity=Severity.WARNING if weekly_trend["trend"] == "increasing" else Severity.SUCCESS
            ))
    ...
    def _analyze_weekly_trends(self, activities: list[CarbonActivity]) -> dict[str, Any]:
        ...
        last_week = sum(a.carbon_emission for a in activities
                       if two_weeks_ago <= a.date < week_ago)
    ...
    def _analyze_category_breakdown(self, activities: list[CarbonActivity]) -> dict[str, float]:
    ...
    def _detect_anomalies(self, activities: list[CarbonActivity]) -> list[CarbonInsight]:
        ...
        for activity in activities:
            ...
            daily_emissions[day] = daily_emissions.get(day, 0) + activity.carbon_emission

        if daily_emissions:
            avg_daily = sum(daily_emissions.values()) / len(daily_emissions)
            std_dev = (sum((x - avg_daily) ** 2 for x in daily_emissions.values()) / len(daily_emissions)) ** 0.5

            for day, emission in daily_emissions.items():
                if emission > avg_daily + 2 * std_dev:  # 2 standard deviations above mean
                    anomalies.append(CarbonInsight(
                        ...
                        data={"date": str(day), "emission": emission, "average": avg_daily},
...
class RecommendationAgent(BaseAgent):
    ...
    async def execute(self, input_data: dict[str, Any]) -> list[AIRecommendation]:
        ...
        for category, emission in sorted_categories:
            category_recs = self._generate_category_recommendations(category, emission, user)
    ...
    def _calculate_category_totals(self, activities: list[CarbonActivity]) -> dict[str, float]:
    ...
    def _generate_category_recommendations(self, category: str, emission: float, user: User) -> list[AIRecommendation]:
...
class GeminiInsightAgent(BaseAgent):
    ...
    def _prepare_context(self, user: User, activities: list[CarbonActivity]) -> str:
        ...
        for activity in activities:
            category = activity.category.value
            category_breakdown[category] = category_breakdown.get(category, 0) + activity.carbon_emission
...
3. Predictions about potential improvements
    ...
    async def _call_gemini_api(self, context: str, question: Optional[str] = None) -> dict[str, Any]:
...
"""
    ...
    def _fallback_insight(self, user: User, activities: list[CarbonActivity]) -> dict[str, Any]:
...
class AgentOrchestrator:
    ...
    async def execute_workflow(self, input_data: dict[str, Any]) -> dict[str, Any]:
        ...
        tasks = [
            self.agents["carbon-analysis"].execute({"user": user, "activities": activities}),
            self.agents["recommendation"].execute({"user": user, "activities": activities, "insights": []}),
            self.agents["gemini-insight"].execute({"user": user, "activities": activities, "question": question})
        ]

        insights, recommendations, gemini_insight = await asyncio.gather(*tasks)
